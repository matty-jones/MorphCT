...
def stitch_images(montage_dims, images_to_stitch, morphology_name, title):
    ...
    for ID, image in enumerate(images_to_stitch):
        ...
        if image[-4:] == '.pdf':
            ...
            sp.call(["convert", "-density", "500", image, "-resize", "20%",
                     "-font", "Arial-Black", "-pointsize", "10",
                     "-gravity", "NorthWest", "-splice", "0x10%",
                     "-page", "+0+0", "-annotate", "0", str(ID+1) + ")",
            ...
            sp.call(['convert', image, '-resize', '500x500',
                     '-font', 'Arial-Black', '-pointsize', '72',
                     '-gravity', 'NorthWest', '-splice', '100x120',
                     '-page', '+0+0', '-annotate', '+40+0', str(ID+1) + ')',
                     directory + '/' + ID_str + '_temp.png'])
    # Create montage
    print("Creating montage...")
    montage = sp.Popen(["montage", "-mode", "concatenate", "-tile",
                        montage_dims, directory + "/*_temp.png",
                        "miff:-"], stdout=sp.PIPE)
    print("Exporting montage...")
    convert = sp.call(["convert", "miff:-", "-density", "500",
                       "-resize", "2000x", "-font", "Arial-Black",
                       "-pointsize", "10", "-gravity", "North",
                       "-bordercolor", "white", "-border", "0x100",
                       "-annotate", "0", title, directory + '/' \
                       + title.replace(" ", "_") + ".png"],
                      stdin=montage.stdout)
    montage.wait()
    print("Removing temporary files...")
    for file_name in glob.glob(directory + "/*_temp.png")\
                     + glob.glob(directory + "/*_crop.png"):
        os.remove(file_name)
    print("Montage created and saved at " + directory + "/"\
          + title.replace(" ", "_") + ".png")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("-d", "--dimensions", default="x1", required=False,
                        help=("The dimensions flag is used to specify the montage"
    ...
    parser.add_argument("-t", "--title", default=None, required=False,
                        help=("Set a custom title for the montage. If not set, will be"
