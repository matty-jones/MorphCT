...
def check_bonds(morphology, bond_dict):
    for bond in morphology['bond']:
        ...
        if separation >= morphology['lx'] / 2.0:
            print("Periodic bond found:", bond, "because separation =",
                  separation, ">=", morphology['lx'] / 2.0)
...
def main():
    ...
    for file_name in list_of_files:
        ...
        if len(split_file_name) > 1:
            file_directory += '/'
        hf.write_morphology_xml(morphology, file_directory + "image_fix_" + split_file_name[-1])
