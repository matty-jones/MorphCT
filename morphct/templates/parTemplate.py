...
morphology = None   # The name of the morphology to run (needs to match the name of the xml (including file extension) in inputMorphDir.
...
deviceMorphology = 'testCrystalBilayerLog'  # The name of the device morphology to use (must match a directory in the inputDeviceDir).
deviceComponents = {\
0: 'donorCrystal',
1: 'acceptorCrystal',
...
terminationConditions = ['KE_min', 'max_t', 'max_t', 'max_t', 'max_t', 'max_t', 'max_t', 'max_t']  # The conditions under which the simulation finishes. Incorporated to allow simulations to finish when they hit the minimum KE. (permitted: 'KEmin, maxt')
...
targetDoSSTDHOMO = 0.1              # Gaussian width of the donor HOMO density of states (100 meV recommended for polymers, small molecules can be less)
targetDoSSTDLUMO = 0.05             # Gaussian width of the acceptor LUMO density of states (100 meV recommended for polymers, small molecules can be less)
...
koopmansHoppingPrefactor = 1E-3     # When Koopmans' approximation is active, this hopping prefactor (which is dependent on energetic disorder) will be applied, which can be tuned to obtain better agreement with experimental mobilities. NOTE: This hopping prefactor is only considered in the mobility simulations. For device simulations, only the hoppingPrefactor parameter is used (look in the Device KMC Parameters section).
...
simulationTimes = [1.00e-12, 3.98e-12, 1.58e-11, 6.31e-11, 2.51e-10, 1.00e-9]  # The termination condition for the mobility KMC simulations
combineKMCResults = True                # This flag combines the KMC results from multiple cores before termination. Often this doesn't finish in time due to cluster configuration, but there's an anlysis script that does it for you (combineKMC) if it doesn't work.
...
if __name__ == "__main__":
    ...
    procIDs = hf.getCPUCores()
    parameterNames = [i for i in dir() if (not i.startswith('__'))\
                      and (i not in ['run_MorphCT', 'helper_functions', 'sys'])]
    parameters = {}
