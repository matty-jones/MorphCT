...
def get_parameter_lines(morphology_name, input_xml):
    with open('par.py', 'r') as parameter_file:
        ...
        for line_number, line in enumerate(lines):
            if '<INPUTXML>' in line:
                lines[line_number] = ''.join(["'", input_xml, "'"]).join(line.split('<INPUTXML>'))
    new_parameter_location = os.path.join('.', 'submissions',
                                          ''.join(['par_', morphology_name, '.py']))
...
def get_slurm_sub_lines(morphology_name, input_xml, param_file_loc):
    ...
    new_submission_location = os.path.join('.', 'submissions',
                                          ''.join(['sub_', morphology_name, '.sh']))
...
if __name__ == "__main__":
    ...
    for input_xml in files_list:
        ...
        new_submission_location = get_slurm_sub_lines(morphology_name, input_xml,
                                                      parameter_location)
