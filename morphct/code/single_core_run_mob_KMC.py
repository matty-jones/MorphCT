...
class carrier:
    def __init__(self, chromophore_list, parameter_dict, chromo_ID, lifetime,
                 carrier_no, AA_morphology_dict, mol_ID_dict):
        self.ID = carrier_no
        self.image = [0, 0, 0]
        ...
        self.no_hops = 0
        self.sim_dims = [[-AA_morphology_dict['lx'] / 2.0, AA_morphology_dict['lx'] / 2.0],
    ...
    def calculate_hop(self, chromophore_list):
        ...
        if self.use_average_hop_rates is True:
            ...
            for neighbour_details in self.current_chromophore.neighbours:
                ...
                if self.mol_ID_dict[self.current_chromophore.ID] == self.mol_ID_dict[neighbour.ID]:
            ...
            for neighbour_index, transfer_integral in enumerate(self.current_chromophore.neighbours_TI):
                ...
                if self.use_VRH is True:
                    neighbour_chromo = chromophore_list[self.current_chromophore.neighbours[neighbour_index][0]]
...
def save_pickle(save_data, save_pickle_name):
    ...
    hf.write_to_file(log_file, ['Pickle file saved successfully as ' + save_pickle_name + '!'])
...
def initialise_save_data(n_chromos, seed):
    return {'seed': seed, 'ID': [], 'image': [], 'lifetime': [], 'current_time': [], 'no_hops': [],
            'displacement': [], 'hole_history_matrix': lil_matrix((n_chromos, n_chromos), dtype=int),
            'electron_history_matrix': lil_matrix((n_chromos, n_chromos), dtype=int),
            'initial_position': [], 'final_position': [], 'carrier_type': []}
...
if __name__ == '__main__':
    ...
    try:
        for job_number, [carrier_no, lifetime, carrier_type] in enumerate(jobs_to_run):
            ...
            this_carrier = carrier(chromophore_list, parameter_dict, start_chromo_ID, lifetime,
                                   carrier_no, AA_morphology_dict, mol_ID_dict)
            terminate_simulation = False
            while terminate_simulation is False:
                terminate_simulation = bool(this_carrier.calculate_hop(chromophore_list))
            ...
            if (t2 - save_time) > 3600:
                print("Completed", job_number, "of", len(jobs_to_run),
                      "jobs. Making checkpoint at %3d%%" % (np.round((job_number + 1)
                                                                     / float(len(jobs_to_run)) * 100)))
                hf.write_to_file(log_file, ['Completed ' + str(job_number)
