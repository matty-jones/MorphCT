...
class morphology:
    ...
    def analyse_morphology(self):
        ...
        molecule = []
        unique_mappings = []
        CG_sites, mappings = helper_functions.parallel_sort(list(new_type_mappings.keys()),
                                                            list(new_type_mappings.values()))
        ...
        for index, CG_sites in enumerate(molecule):
            printMol = True
            initial_atoms, final_atoms = helper_functions.parallel_sort(
                list(unique_mappings[index].keys()), list(unique_mappings[index].values()))
            for index, initial_atom in enumerate(initial_atoms):
                if initial_atom == final_atoms[index]:
                    continue
                if printExplanation is True:
                    print("The following atom types have been remapped due to conflicting"
        ...
        for molecule_number in range(len(molecule_IDs)):
            ...
            AA_molecule_dict, C_gto_AAIDs, ghost_dictionary = atomistic(
                ...
                self.parameter_dict).return_data()
        ...
        for bond_no, bond in enumerate(ghost_dictionary['bond']):
            ...
            if str(bond[2])[0] == '*':
                ghost_dictionary['bond'][bond_no][2] = int(bond[2][1:]) + total_number_of_atoms
        ...
        pickle_location = self.output_morph_dir + '/' + self.morphology_name + '/code/'\
            + self.morphology_name + '.pickle'
        helper_functions.write_pickle((AA_morphology_dict, self.CG_dictionary, CG_to_AAID_master,
                                       self.parameter_dict, self.chromophore_list), pickle_location)
        return AA_morphology_dict, self.CG_dictionary, CG_to_AAID_master, self.parameter_dict,\
    ...
    def get_new_type_mappings(self, CG_to_template_dirs, CG_to_template_force_fields):
        ...
        for FF_loc in force_field_locations:
            mapping_for_this_FF = {}
            force_field = helper_functions.load_FFXML(FF_loc)
        ...
        for CG_site, directory in CG_to_template_dirs.items():
            FF_loc = directory + '/' + CG_to_template_force_fields[CG_site]
            CG_to_template_mappings[CG_site] = force_field_mappings[force_field_locations.index(FF_loc)]
        return CG_to_template_mappings


class atomistic:
    def __init__(self, molecule_index, site_IDs, CG_dictionary, molecule_lengths, rolling_AA_index,
                 ghost_dictionary, parameter_dict):
        ...
        for CG_atom_type in list(self.CG_to_template_files.keys()):
            template_dictionary = helper_functions.load_morphology_XML(self.CG_to_template_dirs[CG_atom_type]
                                                                       + '/' + self.CG_to_template_files[CG_atom_type])
            template_dictionary = self.remap_atom_types(template_dictionary,
                                                        parameter_dict['new_type_mappings'][CG_atom_type])
            template_dictionary = helper_functions.add_unwrapped_positions(template_dictionary)
            self.AA_templates_dictionary[CG_atom_type] = template_dictionary
        self.AA_dictionary, self.atom_ID_lookup_table, self.ghost_dictionary = self.run_fine_grainer(ghost_dictionary)
    ...
    def get_CG_monomer_dict(self):
        ...
        for key in ['bond', 'angle', 'dihedral', 'improper']:
            for element in self.CG_dictionary[key]:
                for site_ID in self.site_IDs:
                    if (site_ID in element) and (element not in CG_monomer_dictionary[key]):
                        CG_monomer_dictionary[key].append(element)
        # Now update the box parameters
        for key in ['lx', 'ly', 'lz']:
            CG_monomer_dictionary[key] = self.CG_dictionary[key]
        CG_monomer_dictionary = helper_functions.add_unwrapped_positions(CG_monomer_dictionary)
    ...
    def remap_atom_types(self, template_dict, mappings):
        ...
        for constraint_type in constraint_types:
            for constraint_index, constraint in enumerate(template_dict[constraint_type]):
                new_constraint0 = ""  # create a new constraint label
                for atom_ID in constraint[1:]:
                    new_constraint0 += template_dict['type'][atom_ID]
                    new_constraint0 += '-'
                    # Assign the constraint label
                template_dict[constraint_type][constraint_index][0] = new_constraint0[:-1]
    ...
    def run_fine_grainer(self, ghost_dictionary):
        ...
        for monomer_no, monomer in enumerate(monomer_list):
            ...
            for CG_site in monomer:
                template_files.append(self.CG_to_template_files[self.CG_dictionary['type'][CG_site]])
                monomer_CG_types.append(self.CG_dictionary['type'][CG_site])
            if len(set(template_files)) != 1:
                print(monomer)
                print(monomerCGTypes)
                print(templateFiles)
                raise system_error('not all MONOMER sites are the same template')
            ...
            if len(this_monomer_dictionary['image']) == 0:
                this_monomer_dictionary['image'] = [[0, 0, 0]] * len(this_monomer_dictionary['position'])
            for site_ID in monomer:
                ...
                atom_ID_lookup_table[site_ID] = [CG_type_list[site_ID],
                ...
                for AAID in self.CG_to_template_AAIDs[CG_type_list[site_ID]]:
                    this_monomer_dictionary['unwrapped_position'][AAID] = list(np.array(
                ...
                if CG_type_list[site_ID] in self.rigid_body_sites:
                    ...
                    for AAID, AA_position in enumerate(this_monomer_dictionary['unwrapped_position']):
                        separation = helper_functions.calculate_separation(
                            self.CG_dictionary['unwrapped_position'][site_ID], AA_position)
            ...
            AA_dictionary = self.update_molecule_dictionary(this_monomer_dictionary, AA_dictionary)
    ...
    def add_terminating_to_molecule(self, AA_dictionary, new_hydrogen_data):
        ...
        for hydrogen in new_hydrogen_data:
            ...
            AA_dictionary['bond'].append([AA_dictionary['type'][hydrogen[0]] + '-h1', hydrogen[0], hydrogen_ID])
