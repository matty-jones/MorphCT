...
class md_phase:
    def __init__(self, AA_morphology_dict, CG_morphology_dict, CG_to_AAID_master, parameter_dict,
                 phase_number, input_file, output_file, s_scale, e_scale):
    ...
    def optimise_structure(self):
        ...
        if self.dcd_file_write is True:
            if self.dcd_file_dumpstep is not 0:
                ...
                self.dump_dcd = dump.dcd(filename=self.output_file.replace('xml', 'dcd'),
                                         period=self.duration / 100.0, overwrite=True)
        else:
            self.dump_dcd = None
            ...
            if self.pair_type == 'dpd':
                ...
                for atom_index1, atom_type1 in enumerate([coeff[0] for coeff in self.dpd_coeffs]):
                    for atom_index2, atom_type2 in enumerate([coeff[0] for coeff in self.dpd_coeffs]):
                        self.pair_class.pair_coeff.set(
                            ...
                            A=np.sqrt((self.dpd_coeffs[atom_index1][1] * self.e_scale) * (
                                self.dpd_coeffs[atom_index2][1] * self.e_scale)),
                            r_cut=np.sqrt((self.dpd_coeffs[atom_index1][2] * self.s_scale) * (
                ...
                for atom_index1, atom_type1 in enumerate([coeff[0] for coeff in self.lj_coeffs]):
                    for atom_index2, atom_type2 in enumerate([coeff[0] for coeff in self.lj_coeffs]):
                        self.pair_class.pair_coeff.set(
                            atom_type1, atom_type2,
                            epsilon=np.sqrt((self.lj_coeffs[atom_index1][1] * self.e_scale) * (
                                self.lj_coeffs[atom_index2][1] * self.e_scale)),
                            sigma=np.sqrt((self.lj_coeffs[atom_index1][2] * self.s_scale) * (
                ...
                raise SystemError("Non-dpd/lj pair potentials not yet hard-coded!"
        ...
        # bond k values to 0.
            ...
            for bond_type in self.AA_morphology_dict['bond']:
                if 'X' in bond_type[0]:
                    ...
                    if (atom_type1 in group_anchoring_types) or (atom_type2 in group_anchoring_types):
            ...
            raise SystemError("Non-harmonic bond potentials not yet hard-coded!"
                              " Please describe how to interpret them on this line.")
        # Set Angle Coeffs
            ...
            if self.angle_type == 'harmonic':
                ...
                for angle_coeff in self.angle_coeffs:
                    ...
                    self.angle_class.set_coeff(
                        angle_coeff[0], k=angle_coeff[1] * self.e_scale, t0=angle_coeff[2])
            else:
                raise SystemError("Non-harmonic angle potentials not yet hard-coded!"
                                  " please describe how to interpret them on this line.")
        else:
            ...
            if self.dihedral_type == 'table':
                ...
                for dihedral_coeff in self.dihedral_coeffs:
                    self.dihedral_class.dihedral_coeff.set(
                        ...
                        coeff=dict(v0=dihedral_coeff[1] * self.e_scale,
                ...
                for dihedral_coeff in self.dihedral_coeffs:
                    self.dihedral_class.set_coeff(
                        dihedral_coeff[0],
                        k1=dihedral_coeff[1] * self.e_scale, k2=dihedral_coeff[2] * self.e_scale,
                        k3=dihedral_coeff[3] * self.e_scale, k4=dihedral_coeff[4] * self.e_scale)
            else:
                raise SystemError("Non-tabulated dihedral potentials not yet hard-coded!"
                                  " Please describe how to interpret them on this line.")
        else:
    ...
    def get_integration_groups(self):
        ...
        non_rigid_group = group.difference(name="non_rigid_group", a=integrate_group, b=rigid_group)
...
def execute(AA_morphology_dict, CG_morphology_dict, CG_to_AAID_master, parameter_dict, chromophore_list):
    ...
    if (parameter_dict['overwrite_current_data'] is False)\
       and ('phase0_' + parameter_dict['morphology'] in current_files):
        pass
    else:
        scale_morphology(AA_morphology_dict, parameter_dict, s_scale, e_scale)
    # Reset logfile
    try:
        os.remove(parameter_dict['output_morph_dir'] + '/' + parameter_dict['morphology'][:-4]
                  + '/morphology/energies_' + parameter_dict['morphology'][:-4] + '.log')
    except OSError:
        pass
    ...
    for key in atom_constraints:
        ...
        for constraint_no, constraint in enumerate(final_morphology[key]):
            for atom_ID in constraint[1:]:
                if (atom_ID in atom_IDs_to_remove) and (constraint_no not in constraints_to_remove):
