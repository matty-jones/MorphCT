...
@pytest.fixture(scope='module',
                params=[' '.join(['-m', 'DBP', os.path.join(
                    TEST_ROOT, 'output_AH', 'DBP.xml')]),
                    ' '.join(['-m', 'P3HT', os.path.join(
                        TEST_ROOT, 'output_AH', 'P3HT.xml')]),
                    ' '.join(['-m', 'P3HT/PCBM', os.path.join(
                        TEST_ROOT, 'output_AH', 'PC60BM.xml')]),
                    ' '.join(['-m', 'P3HT/PCBM', os.path.join(
                        TEST_ROOT, 'output_AH', 'PC70BM.xml')]),
                    ' '.join(['-m', 'PERYLENE', os.path.join(
                        TEST_ROOT, 'output_AH', 'PE.xml')]),
                    ' '.join(['-m', 'PERYLENE', os.path.join(
                        TEST_ROOT, 'output_AH', 'PT.xml')]),
                    ' '.join(['-m', 'BDT-TPD', os.path.join(
                        TEST_ROOT, 'output_AH', 'BDTTPD.xml')])]
               )
def run_simulation(request):
    ...
    shutil.copy(os.path.join(TEST_ROOT, 'assets', 'add_hydrogens',
                             molecule_file),
...
class TestCompareOutputs(TestCommand):
    def test_check_output_exists(self, run_simulation):
        output_file = os.path.split(run_simulation)[1].replace(
            '.xml', '_AA.xml')
    ...
    def test_check_output_identical(self, run_simulation):
        output_file = os.path.split(run_simulation)[1].replace(
            '.xml', '_AA.xml')
        response_location = os.path.join(TEST_ROOT, 'output_AH', output_file)
        expected_location = os.path.join(TEST_ROOT, 'assets', 'add_hydrogens',
                                         output_file)
